...
function Index() {
  ...
  return (
    <div className="relative min-h-screen bg-background">
      ...
      {/* HALL OF CHAMPIONS — Aarav's journey */}
      <section id="memories" className="hoc-section relative overflow-hidden py-28">
        {/* Warm exhibition-hall backdrop */}
        <div className="pointer-events-none absolute inset-0" aria-hidden>
          <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse at 20% 0%, oklch(0.35 0.08 60 / 0.55), transparent 55%), radial-gradient(ellipse at 80% 100%, oklch(0.30 0.10 40 / 0.45), transparent 55%), linear-gradient(180deg, oklch(0.12 0.02 40) 0%, oklch(0.08 0.02 30) 100%)" }} />
          <div className="absolute inset-x-0 bottom-0 h-40" style={{ background: "linear-gradient(180deg, transparent, oklch(0.22 0.06 45 / 0.7))" }} />
          {/* Floating gold particles */}
          {Array.from({ length: 22 }).map((_, i) => (
            <span
              key={i}
              className="hoc-particle"
              style={{
                left: `${(i * 47) % 100}%`,
                top: `${(i * 31) % 100}%`,
        ...
        <div className="relative mx-auto max-w-7xl px-6">
          ...
          <div className="relative mb-20 text-center">
            <div className="hoc-spotlight" aria-hidden />
            <div className="font-mono text-[11px] uppercase tracking-[0.5em] text-amber-200/70">— Exhibition 04 —</div>
            <h2 className="hoc-title mt-4 font-display text-5xl uppercase leading-[1.05] md:text-7xl">
              <span className="block">Aarav's</span>
              <span className="block">Hall of Champions</span>
            </h2>
            <div className="mx-auto mt-5 flex max-w-lg items-center gap-4">
              <span className="hoc-divider" />
              <span className="whitespace-nowrap font-mono text-[11px] uppercase tracking-[0.4em] text-amber-100/80">10 Years of Amazing Memories</span>
              <span className="hoc-divider" />
            </div>
          </div>

          {/* Golden ribbon spine + milestones */}
          <div className="relative">
            <div className="pointer-events-none absolute left-1/2 top-0 hidden h-full w-px -translate-x-1/2 md:block" aria-hidden>
              <div className="hoc-ribbon h-full w-full" />
            </div>

            <ol className="relative space-y-16 md:space-y-24">
              {[
                { age: "01", icon: "🏆", title: "First Steps",        caption: "Every champion starts with their very first step." },
                { age: "03", icon: "🎈", title: "Endless Smiles",     caption: "Laughter became the sound of every afternoon." },
                { age: "05", icon: "🚲", title: "Little Explorer",    caption: "Adventure became his favorite hobby." },
                { age: "07", icon: "🌟", title: "Growing Brighter",   caption: "Kindness and curiosity taking the lead." },
                { age: "09", icon: "🎁", title: "Dream Chaser",       caption: "Bigger dreams, brighter grid lights ahead." },
              ].map((m, i) => {
                const flip = i % 2 === 1;
                return (
                  <li
                    key={m.age}
                    className={`hoc-milestone group relative grid items-center gap-8 md:grid-cols-2 md:gap-16 ${flip ? "md:[direction:rtl]" : ""}`}
                    style={{ ["--i" as never]: i } as React.CSSProperties}
                  >
                    {/* Frame */}
                    <div className="relative md:[direction:ltr]">
                      <div className="hoc-frame group-hover:hoc-frame--hover relative aspect-[4/5] overflow-hidden rounded-2xl">
                        <img src={helmet} alt={m.title} loading="lazy" className="h-full w-full object-cover transition-transform duration-[1200ms] ease-out group-hover:scale-[1.06]" />
                        {/* Warm vignette */}
                        <div className="pointer-events-none absolute inset-0" style={{ background: "radial-gradient(ellipse at 50% 20%, transparent 40%, oklch(0.12 0.03 40 / 0.75) 100%)" }} />
                        {/* Moving reflection */}
                        <div className="hoc-shine pointer-events-none absolute inset-0" aria-hidden />
                        {/* Sparkles on hover */}
                        <div className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-500 group-hover:opacity-100" aria-hidden>
                          {Array.from({ length: 8 }).map((_, s) => (
                            <span key={s} className="hoc-sparkle" style={{ left: `${(s * 37) % 100}%`, top: `${(s * 53) % 100}%`, animationDelay: `${s * 0.15}s` }} />
                          ))}
                        </div>
                        {/* Corner age plate */}
                        <div className="absolute left-4 top-4 flex items-center gap-2 rounded-full border border-amber-300/40 bg-black/40 px-3 py-1.5 backdrop-blur-md">
                          <span className="h-1.5 w-1.5 rounded-full bg-amber-300 shadow-[0_0_8px_theme(colors.amber.300)]" />
                          <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-amber-100">Age {m.age}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Caption */}
                    <div className="relative md:[direction:ltr]">
                      <div className="hoc-plaque rounded-2xl p-7 md:p-9">
                        <div className="flex items-center gap-3">
                          <span className="text-3xl drop-shadow-[0_0_12px_rgba(251,191,36,0.5)]">{m.icon}</span>
                          <span className="font-mono text-[10px] uppercase tracking-[0.4em] text-amber-200/80">Milestone · {m.age}</span>
                        </div>
                        <h3 className="mt-3 font-display text-3xl uppercase leading-tight text-amber-50 md:text-4xl" style={{ textShadow: "0 0 24px rgba(251,191,36,0.25)" }}>
                          {m.title}
                        </h3>
                        <p className="mt-3 max-w-md text-base leading-relaxed text-amber-100/75">
                          {m.caption}
                        </p>
                        <div className="mt-5 flex items-center gap-3">
                          <span className="h-px flex-1 bg-gradient-to-r from-amber-300/60 to-transparent" />
                          <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-amber-200/60">Memory reel</span>
                        </div>
                      </div>
                    </div>

                    {/* Ribbon node */}
                    <span className="pointer-events-none absolute left-1/2 top-1/2 hidden h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full md:block" style={{ background: "radial-gradient(circle, #fde68a 0%, #f59e0b 55%, transparent 70%)", boxShadow: "0 0 24px rgba(251,191,36,0.7)" }} aria-hidden />
                  </li>
                );
              })}
            </ol>
          ...
          <div className="relative mt-24">
            <div className="hoc-finale relative overflow-hidden rounded-3xl p-1">
              <div className="relative grid gap-8 rounded-[22px] bg-gradient-to-br from-[oklch(0.14_0.03_50)] to-[oklch(0.10_0.02_40)] p-8 md:grid-cols-[1.1fr_1fr] md:p-12">
                <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-amber-300/40 md:aspect-auto">
                  <img src={helmet} alt="Aarav today" className="h-full w-full object-cover" />
                  <div className="pointer-events-none absolute inset-0" style={{ background: "radial-gradient(ellipse at 50% 30%, transparent 45%, rgba(0,0,0,0.7) 100%)" }} />
                  <div className="hoc-shine pointer-events-none absolute inset-0" aria-hidden />
                </div>
                <div className="flex flex-col justify-center">
                  <div className="font-mono text-[11px] uppercase tracking-[0.5em] text-amber-300">Today · Age 10</div>
                  <h3 className="mt-3 font-display text-5xl uppercase leading-[1.02] text-amber-50 md:text-6xl" style={{ textShadow: "0 0 30px rgba(251,191,36,0.35)" }}>
                    10 Amazing Years
                  </h3>
                  <p className="mt-4 text-lg text-amber-100/80">
                    Ready for the biggest celebration yet — the grid is lit, the crowd is waiting, and the champion has just arrived.
                  </p>
                  <div className="mt-6 flex flex-wrap items-center gap-3">
                    <span className="rounded-full border border-amber-300/50 bg-amber-300/10 px-4 py-2 font-mono text-[11px] uppercase tracking-[0.3em] text-amber-100">🏁 Ready for the Race</span>
                    <a href="#rsvp" className="rounded-full border border-amber-300/60 bg-gradient-to-r from-amber-400 to-amber-200 px-5 py-2 font-mono text-[11px] uppercase tracking-[0.3em] text-black transition-transform hover:scale-[1.03]">
                      Reserve Your Seat →
                    </a>
                  </div>
                </div>
        ...
        <style>{`
          ...
          .hoc-spotlight {
            position: absolute; left: 50%; top: -40px; width: 620px; height: 320px;
            transform: translateX(-50%);
            background: radial-gradient(ellipse at center, rgba(253,230,138,0.28), transparent 65%);
            filter: blur(10px); pointer-events: none;
            animation: hoc-spot 7s ease-in-out infinite;
          }
          @keyframes hoc-spot { 0%,100% { opacity:.7; transform: translate(-50%,0);} 50% { opacity:1; transform: translate(-48%, 6px);} }
          .hoc-divider { flex:1; height:1px; background: linear-gradient(90deg, transparent, rgba(253,230,138,.6), transparent); }
          .hoc-ribbon {
            background: linear-gradient(180deg, transparent 0%, rgba(253,230,138,.15) 8%, rgba(251,191,36,.85) 50%, rgba(253,230,138,.15) 92%, transparent 100%);
          ...
          .hoc-particle {
            ...
            box-shadow: 0 0 8px rgba(253,230,138,.8);
            animation: hoc-drift linear infinite;
            opacity: .55;
          }
          @keyframes hoc-drift {
            0%   { transform: translate(0,0) scale(.9); opacity:0; }
            15%  { opacity:.7; }
            100% { transform: translate(30px,-140px) scale(1.2); opacity:0; }
          }
          .hoc-frame {
            border: 1px solid rgba(253,230,138,.45);
            box-shadow:
              0 0 0 1px rgba(253,230,138,.1) inset,
              0 30px 60px -20px rgba(0,0,0,.7),
              0 0 60px -15px rgba(251,191,36,.35);
            background: linear-gradient(135deg, rgba(253,230,138,.08), rgba(0,0,0,0));
            transition: transform .7s cubic-bezier(.16,1,.3,1), box-shadow .7s ease;
          }
          .hoc-milestone:hover .hoc-frame {
            transform: translateY(-6px);
            box-shadow:
              0 0 0 1px rgba(253,230,138,.35) inset,
              0 40px 80px -20px rgba(0,0,0,.8),
              0 0 90px -10px rgba(251,191,36,.55);
          }
          .hoc-shine::before {
          ...
          .hoc-milestone:hover .hoc-shine::before,
          .hoc-finale:hover .hoc-shine::before { transform: translateX(120%); }
          .hoc-plaque {
            background: linear-gradient(180deg, rgba(30,20,10,.55), rgba(15,10,5,.55));
            border: 1px solid rgba(253,230,138,.28);
            backdrop-filter: blur(14px);
            box-shadow: 0 20px 50px -20px rgba(0,0,0,.7), inset 0 1px 0 rgba(253,230,138,.15);
          }
          .hoc-sparkle {
            position:absolute; width:4px; height:4px; border-radius:9999px;
            background: #fff7d6; box-shadow: 0 0 10px #fde68a, 0 0 20px #f59e0b;
            animation: hoc-sparkle 1.4s ease-in-out infinite;
          }
          @keyframes hoc-sparkle {
            0%,100% { transform: scale(0); opacity:0; }
            50%     { transform: scale(1); opacity:1; }
          }
          .hoc-finale {
            background: linear-gradient(135deg, #fde68a, #f59e0b 40%, #b45309 70%, #fde68a);
            background-size: 200% 200%;
            animation: hoc-foil 8s ease-in-out infinite;
            box-shadow: 0 40px 100px -20px rgba(0,0,0,.7), 0 0 80px -20px rgba(251,191,36,.55);
          }
          @keyframes hoc-foil { 0%,100% { background-position: 0% 50%;} 50% { background-position: 100% 50%;} }
        `}</style>
      </section>
